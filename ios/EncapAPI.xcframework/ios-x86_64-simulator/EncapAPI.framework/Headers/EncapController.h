...
/**
 * EncapController can be Initialized with a name in order to support multiple active registrations in one application.
 *
 * @Note If the application only support one registration, sharedController can be used.
 *
 * @param name the unique identifier chosen by the developer for a given registration.
 *
 */
...
@property (readwrite, retain, nonnull) EncapConfig *config;
 ...
 */
@property (assign, readonly) BOOL isActivated DEPRECATED_MSG_ATTRIBUTE("Use isActivatedLocallyWithError: instead");
...
@property (readonly, nonnull) EncapRiskParameterServerConfig *riskparameterServerConfig;

/**
 * Checks if activation data is present on the device.
 * It will not perform a call to Encap Server to check status of current activation.
 *
 * @param error If populated with an NSError after call has returned, the operation has failed due to an unexpected error
 *
 * @return NSNumber containing a boolValue, nil if an unexpected error occur during check.
 *
 * @Discussion If operation fails with EncapStorageUnavailable. It could be worth trying one extra call to this method before reactivating since the cause might be that the OS hadn't unlocked the keychain yet (more likely on registrations activated on SDK version 3.5 and earlier).
 *
 * @Note This method throws/returns an NSError instead of an EncapErrorResult. Assuming most (new) apps will use this API from Swift, it has been optimized to use from Swift rather than Objective C.
 * This method will automatically be translated to a Swift throwing method since it returns an NSError, see:
 * https://developer.apple.com/documentation/swift/cocoa_design_patterns/about_imported_cocoa_error_parameters
 * NSError code property will be equivalent to errorCode in EncapErrorResult. Possible to convert to EncapErrorResult with initWithNSError: method.
 */
- (nullable NSNumber *) isActivatedLocallyWithError:(NSError **)error;
 ...
 * be deactivated on encap server, if device authentication is successful local registration data will be 
 ...
 * @param onError The block that gets executed if operation fails.
 */
- (void) startAddOrUpdateOfAuthMethodWithClientData:(nullable NSString *)clientData
